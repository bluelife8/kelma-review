...
private object DesktopLuaLibrary {
    init {
        val mappedName = System.mapLibraryName("kelma_lua")
        val resource = requireNotNull(DesktopLuaLibrary::class.java.getResourceAsStream("/native/$mappedName")) {
            "Embedded Lua 5.4 runtime is unavailable for this desktop platform"
        }
        val extracted = Files.createTempFile("kelma-lua-", mappedName.substringAfterLast('.').let { ".$it" })
        resource.use { input -> Files.newOutputStream(extracted).use(input::copyTo) }
...
internal class LuaNativeBridge(
    ...
    init {
        DesktopLuaLibrary.ensureLoaded()
